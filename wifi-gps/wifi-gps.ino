...
// sensor IDs
// Temperatur
const char TEMPERSENSOR_ID[] PROGMEM = "";
// rel. Luftfeuchte
const char RELLUFSENSOR_ID[] PROGMEM = "";
// Luftdruck
const char LUFTDRSENSOR_ID[] PROGMEM = "";
// Beleuchtungsstärke
const char BELEUCSENSOR_ID[] PROGMEM = "";
// UV-Intensität
const char UVINTESENSOR_ID[] PROGMEM = "";
// PM10
const char PM10SENSOR_ID[] PROGMEM = "";
// PM2.5
const char PM25SENSOR_ID[] PROGMEM = "";
...
char valBuffer[10];
char tsBuffer[21];
char tsLatLngBuffer[42];

#define GPS_SERIAL Serial2

float lat, lng;

void fillTsLatLngBuffer() {
  dtostrf(lat, 9, 6, latBuffer);
  dtostrf(lng, 9, 6, lngBuffer);
  sprintf_P(tsLatLngBuffer, PSTR("%s,%s,%s"), tsBuffer, lngBuffer, latBuffer);
...
void writeMeasurementsToClient() {
  ...
  for (uint8_t i = 0; i < num_measurements; i++) {
    ...
    dtostrf(measurements[i].value, 9, 2, valBuffer);

    sprintf(buffer, "%S,%s,%s", measurements[i].sensorId, valBuffer, tsLatLngBuffer);
    ...
    if (connected == true) {
      ...
      sprintf_P(buffer,
                PSTR("POST /boxes/%S/data HTTP/1.1\nHost: %S\nContent-Type: "
                     "csv\nConnection: close\nContent-Length: %i\n"),
                SENSEBOX_ID, server, num_measurements * 77);
      // 36 old
...
void setup() {
  ...
  while (!GPS_SERIAL.available()) {
    DEBUG(F("detecting GPS device..."));
    delay(1000);
  }
  DEBUG(F("Wait for GPS..."));
